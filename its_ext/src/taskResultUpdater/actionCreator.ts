...
function generateUpdateTaskResultActions(
    ...
    for (const personalNumber in personalNumberToTaskResult) {
        ...
        if (!admissionInfo[studentAdmissionId].hasOwnProperty(personalNumber)) {
            ...
            throw Error(`personalNumber: ${personalNumber} not found in admissionInfos ${admissionIdsStr}`);
        }
        ...
        actions.push(new UpdateTaskResultAction(
            student.id, studentAdmissionId, newTaskResult
        ));
    }
    return actions;
}
...
export function createTaskResultActions(
    ...
    if (admissionIds.length !== 2) {
        throw Error(`admissionIds must have length 2, but got ${admissionIds.length}`);
    }

    actions.push(...generateUpdateTaskResultActions(
        admissionIds, admissionInfo, personalNumberToTaskResult,
        studentData
    ));
